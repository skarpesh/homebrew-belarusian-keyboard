...
Quotation marks sit on the same keys in both layouts: `[` opens and `]`
closes, like the brackets themselves — ⌥ gives the outer « », ⇧⌥ the nested
„ “ — and ⌥ + `p` the single ‘ ’. The typographic apostrophe ’ sits on ⇧ + `'`
on the Cyrillic layout and on ⌥ + `'` on the Latin one; there ⇧⌥ + `'`
doubles it into ” — same single-to-double move as `'` → `"` on that key —
completing the American “ ” and Polish „ ” pairs.
